...
#class WindowAttention(LightningModule):
...
#class SwinTransformerBlock(LightningModule):
...
#class PatchMerging(LightningModule):
...
#class BasicLayer(LightningModule):
...
#class PatchEmbed(LightningModule):
...
#class SwinTransformer(LightningModule):
    ...
    def forward(self, x):
        ...
        #x = self.forward_features_multi(x)
        x = self.head(x)
        return x
...
class SwinBackboneForSegmentation(SwinTransformer):
    ...
    def forward_features(self, x):
        features = []
        x = self.patch_embed(x) # some flattening in function
        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x) # shape remains

        for layer in self.layers:
            x = layer(x)
            x_unflatten = torch.unflatten(x, 1,(int(x.shape[1]**0.5), int(x.shape[1]**0.5)))
            features.append(x_unflatten)
        # x = self.norm(x)  # B L C
        # x = self.avgpool(x.transpose(1, 2))  # B C 1
        # x = torch.flatten(x, 1)
...
def remap_pretrained_keys_swin(model, checkpoint_model, logger):
    ...
    #logger.info('State dict: %s' % str(state_dict.keys()))

    # Geometric interpolation when pre-trained patch size mismatch with fine-tuned patch size
    all_keys = list(checkpoint_model.keys())
    #logger.info('All keys: %s' % str(all_keys))
    #exit()
    for key in all_keys:
        #key2 = key.replace(["proj","attn","layers"], ["projection","attn.w_msa","stages"])
        if "relative_position_bias_table" in key:
            # stages.0.blocks.0.attn.w_msa.relative_position_bias_table
            # layers.0.blocks.0.attn.relative_position_bias_table
            #key2 = key.replace("layers", "stages")
            #key2 = key2.replace("attn", "attn.w_msa")
            relative_position_bias_table_pretrained = checkpoint_model[key]
            #relative_position_bias_table_current = state_dict[key2]
...
def load_pretrained(config, model, logger):
    ...
    #### rename keys ####
    # if any([True if (x in k for x in ['projection', 'stages', 'attn.w_msa']) else False for k in checkpoint_model.keys()]):
    #     #print("checkpoint model: %s" % checkpoint_model.keys())
    #     #print("model: %s" % model.state_dict().keys())
    #     checkpoint_model = {k.replace('proj','projection').replace('layers','stages').replace('attn','attn.w_msa'): v for k, v in checkpoint_model.items()}
    #
    #     #exit()
    ...
    print("Verfügbare Attribute in patch_embed:", dir(model.patch_embed))
    ...
    #     if model.patch_embed.projection.weight.shape != checkpoint_model['patch_embed.proj.weight'].shape:
    #         temp = model.patch_embed.projection.weight.data.cpu()
    #         if checkpoint_model['patch_embed.proj.weight'].shape[1] == 1:
    #             # greyscale pretrained model
    #             temp = checkpoint_model['patch_embed.proj.weight'].repeat(1, temp.shape[1], 1, 1)
    #         elif checkpoint_model['patch_embed.proj.weight'].shape[1] == 12 and temp.shape[1] == 3:
...
def main(config):
    ...
    print(f"Input shape of second sample: {val_dataset[1]['image'].shape}")
    ...
    print(f"Input shape of second sample: {train_dataset[1]['image'].shape}")
    print(f"Input shape of third sample: {train_dataset[2]['image'].type()}")
    ...
    ##### Load pretrained FM to backbone #####
    if config.PRETRAINED: # and config.MODEL.RESUME == "":
        load_pretrained(config, backbone, logger)

    decoder = upernet_decoder.UperNetDecoder(embed_dim=[256, 512, 1024, 1024], channels=backbone.num_features)

    model_factory = EncoderDecoderFactory()
    model = model_factory.build_model(task="segmentation", backbone=backbone, decoder=decoder, necks=[{"name": "PermuteDims", "new_order": [0,3,1,2]}],
                                      num_classes=len(config.DATA.CLASSES))
    ...
    logger_tt = TensorBoardLogger(save_dir=default_root_dir, name=experiment)
